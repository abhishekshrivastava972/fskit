...
static enum FSV1ErrorLevel *xmlTextReaderReadFSV1ErrorLevelType(xmlTextReaderPtr reader)
{
  ...
  if (enumValue != NULL) {
    if (xmlStrcmp(enumValue, BAD_CAST "Info") == 0) {
      *value = RESERVATION_FSAPI_V1_ERRORLEVEL_INFO;
      free(enumValue);
      return value;
    }
    if (xmlStrcmp(enumValue, BAD_CAST "Warn") == 0) {
      *value = RESERVATION_FSAPI_V1_ERRORLEVEL_WARN;
      free(enumValue);
      return value;
    }
    if (xmlStrcmp(enumValue, BAD_CAST "Error") == 0) {
      *value = RESERVATION_FSAPI_V1_ERRORLEVEL_ERROR;
      free(enumValue);
      return value;
    }
#if DEBUG_ENUNCIATE
...
{
  switch (*_errorLevel) {
    case RESERVATION_FSAPI_V1_ERRORLEVEL_INFO:
      return xmlTextWriterWriteString(writer, BAD_CAST "Info");
    case RESERVATION_FSAPI_V1_ERRORLEVEL_WARN:
      return xmlTextWriterWriteString(writer, BAD_CAST "Warn");
    case RESERVATION_FSAPI_V1_ERRORLEVEL_ERROR:
